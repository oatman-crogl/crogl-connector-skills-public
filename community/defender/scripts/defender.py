#!/usr/bin/env python3
"""crogl-defender CLI — drives a Microsoft Defender for Endpoint custom
connector via the MCP api_proxy tool.

Subcommands:
  query          OData $filter search over a resource (alerts|machines).
  ...
  ls              List this skill's Defender connectors' database schema by UNIX path.
  grep            String-search this skill's connectors' KG catalog.

The configured custom connector's base URL should be the Defender for
Endpoint API root (https://api.securitycenter.microsoft.com). Auth (an OAuth
client_credentials Bearer) is applied server-side from the connector's
stored secret.
"""
...
MAX_RESULT_SETS = 10
VIEW_TOOL_NAME = "defender.py view-result"
BASE_URL = "https://api.securitycenter.microsoft.com"

# Result-size ceiling (CONTRIBUTING.md "Every list/query command has a
# code-enforced result-size ceiling"). Both resources return full entities
# in one page-sized response with no separate bulk-listing command, so one
# ceiling applies everywhere. MAX_PAGE_SIZE is the explicit $top sent on
# every request -- capped at HARD_CAP since no query here ever needs more
# than one page's worth beyond that.
DEFAULT_LIMIT = 25
HARD_CAP = 100
MAX_PAGE_SIZE = HARD_CAP

RESOURCES: dict[str, dict[str, str]] = {
    "alerts": {"list_path": "/api/alerts"},
    "machines": {"list_path": "/api/machines"},
}
...
def _search(
    ...
    """Single-stage Defender API fetch with @odata.nextLink pagination up to
    limit. Like Graph, alerts/machines return full entities directly — no
    separate id -> hydrate stage.
...
class _DefenderSchemaProvider:
    """SchemaProvider for Defender for Endpoint: resource -> field.

    The Defender API has no createmeta-style schema endpoint, so the leaf
    field tier is discovered empirically: a one-row sample of the resource,
    whose top-level JSON keys are the catalog fields.
    """
...
\b
  defender.py query --resource alerts --filter "severity eq 'High'" --limit 50
  defender.py query --resource machines --limit 50
  defender.py get --resource machines --id <id>
...
def cli(ctx: click.Context, json_meta: bool) -> None:
    """Drive a Microsoft Defender for Endpoint connector. Run `<command> --help` for details."""
...
@click.option(
    ...
    default="alerts",
    show_default=True,
    help="Defender resource to query.",
)
@click.option(
    "--filter",
    "odata_filter",
    default="",
    help="OData $filter expression (e.g. \"severity eq 'High'\" or "
    "\"healthStatus eq 'Active'\"). Empty returns all rows up to --limit.",
)
@click.option(
    "--limit",
    required=True,
...
def view_result(ctx: click.Context, ref: str, result_set: int, row: int) -> None:
    ...
    Defender entities are returned in full at search time, so this is a
    pure cache read — no second backend call needed.
    """
    try:
...
@click.option(
    ...
    default="alerts",
    show_default=True,
    help="Defender resource the id belongs to.",
)
...
def _sample_rows(
    ...
    """KG sample/lookup over a Defender resource, in the canonical row shape
...
kgcommands.add_commands(
    ...
    upload_from_file_help="Path to a file written by `defender.py query --output …`.",
    upload_catalog_path_help="POSIX-style catalog path the rows belong to (e.g. /alerts/<field> or /machines/<field>).",
